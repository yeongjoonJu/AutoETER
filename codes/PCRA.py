...
f = open("data/FB15k/relations.dict","r")
relation2id = {}
id2relation = {}
relation_num = 0
for line in f:
    seg = line.strip().split()
    relation2id[seg[1]] = int(seg[0])
    id2relation[int(seg[0])]=seg[1]
    id2relation[int(seg[0])+ relation_num]="~"+seg[1]
    relation_num+=1
f.close()
...
for line in f:
    seg = line.strip().split()
    if (seg[0]+" "+seg[2] not in ok):
        ok[seg[0]+' '+seg[2]]={}
    if (seg[2]+" "+seg[0] not in ok):
        ok[seg[2]+' '+seg[0]]={}
f.close()

h_e_p = {}

f = open("data/FB15k/1-1.txt","r")
for line in f:
    seg = line.strip().split()
    if len(seg)!=2:
        continue
...
def work(dir):
    ...
    for line in f:
        ...
        g.write("\n")
        g.write(str(e2)+" "+str(rel+relation_num)+" "+str(e1)+"\n")
        e1 = seg[1]
        e2 = seg[0]
        b = {}
        a = {}
